...
1. 代码、结构化数据和真实实验图片尚未分别选择开放许可证；公开可见不等于授权复用。
2. 接受偏差的相机内参实体文件尚未取回并归档。
3. `hand_landmarker.task` 的准确下载来源、版本和许可尚未闭环，因此不随仓库分发。
4. 当前没有 Zenodo、OSF 或机构仓库 DOI；投稿时仍需按目标会议要求决定是否建立稳定归档。
